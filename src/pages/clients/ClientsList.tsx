...
import { Search, ArrowUpRight, Building2, User } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-sm flex flex-col md:flex-row gap-3">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Search by business name, PAN, GSTIN, or email..."
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-slate-900">
                            {c.businessName || c.contactPersonName || 'Client'}
                          </p>
                          <p className="text-[10px] text-slate-400">ID: {c.publicId.slice(0, 8)}...</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 font-semibold text-slate-700">
                      <span className="capitalize">{c.clientType.toLowerCase()}</span>
                    </td>
                    <td className="py-3.5 px-4">
                      <p className="font-medium text-slate-900">{c.contactEmail}</p>
                      <p className="text-[10px] text-slate-400">{c.contactPhone || 'No phone'}</p>
                    </td>
                    <td className="py-3.5 px-4 font-mono font-semibold text-slate-800">
                      {c.panNumber || <span className="text-slate-300 italic">N/A</span>}
                    </td>
                    <td className="py-3.5 px-4 font-mono text-[11px] text-slate-800">
                      {c.gstin || <span className="text-slate-300 italic">N/A</span>}
                    </td>
                    <td className="py-3.5 px-4">
